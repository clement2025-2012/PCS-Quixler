...
class PCSQuizApp {
    ...
    init() {
        ...
        // After 3.5s dismiss splash and show home
        setTimeout(() => {
            this.showScreen('home-screen');

            // Push AdSense 600ms after home screen is visible
            setTimeout(() => {
                try {
                    var slot = document.getElementById('pcs-ad-slot');
    ...
    setupEventListeners() {
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if (map[e.key.toLowerCase()] !== undefined) {
                self.selectAnswerByIndex(map[e.key.toLowerCase()]);
    ...
    startQuiz() {
        ...
        // Rebuild quiz HTML if results were shown previously
    ...
    showExplanation(text, isCorrect) {
        ...
        div.innerHTML = '<div class="explanation-header"><span class="explanation-icon">' + (isCorrect ? '&#10003;' : '&#10007;') + '</span><span class="explanation-status">' + (isCorrect ? 'Correct!' : 'Incorrect') + '</span></div><div class="explanation-text">' + text + '</div>';
        var card = document.querySelector('.question-card');
        if (card) {
            card.appendChild(div);
            setTimeout(function() { div.style.opacity = '1'; }, 20);
        }
    }
    ...
    showResults() {
        ...
        var statsHTML = stats.map(function(s) {
            return '<div class="category-stat-item"><span class="category-name">' + s.category + '</span><div class="category-bar"><div class="category-fill" style="width:' + s.percentage + '%"></div></div><span class="category-percentage">' + s.percentage + '%</span></div>';
        }).join('');

        var wrapper = document.querySelector('.quiz-wrapper');
        if (!wrapper) return;
        wrapper.innerHTML = '<div class="results-container">' +
            '<div class="results-header">' +
            '<h1 class="results-title">Quiz Complete! ' + level.emoji + '</h1>' +
            '<div class="results-score-circle"><div class="score-circle-inner"><span class="score-percentage">' + pct + '%</span><span class="score-label">Score</span></div></div>' +
            '</div>' +
            ...
            '<div class="category-performance"><h3>Performance by Category</h3><div class="category-stats">' + statsHTML + '</div></div>' +
    ...
    showNotification(message, type) {
        ...
        n.className = 'pcs-notif pcs-notif-' + type;
    ...
    injectStyles() {
        ...
        s.textContent = [
            ...
            '.pcs-notif button:hover { opacity:1; }',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        r.style.cssText = 'position:absolute;border-radius:50%;pointer-events:none;width:' + size + 'px;height:' + size + 'px;left:' + (e.clientX - rect.left - size/2) + 'px;top:' + (e.clientY - rect.top - size/2) + 'px;background:rgba(255,255,255,.3);transform:scale(0);animation:btnRipple .6s linear;';
